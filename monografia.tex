...
\keyword{wavelet, haar, daubechies, optimization, performance}

\begin{document}

\renewcommand{\advisorname}{Orientadora}           %descomente caso tenhas orientadora
\renewcommand{\coadvisorname}{Coorientador}      %descomente caso tenhas coorientadora
\renewcommand{\collaboratorname}{Colaboradora}
...
\begin{dedicatoria}
  Dedico este trabalho a meu avô, Orlando Ferreira dos Santos, conhecido "Camarão", por ter apoiado meus estudos até o início de minha formação acadêmica. Por conta de eventos inevitáveis da vida, infelizmente não poderei lhe mostrar o diploma, porém registro aqui minha dedicatória ao velho camarada.
\end{dedicatoria}

\begin{epigrafe}
  "Leave this world a little better than you found it."\\
  {\sc --- Baden-Powell's Last Message (1945)}
\end{epigrafe}

\begin{englishabstract}%
{Biblioteca Int-DWTs - Simplificações Algébricas Aumentando Desempenho e Exatidão de Transformadas Discretas Wavelet}
	{wavelet, haar, daubechies, otimização, desempenho}

Este trabalho descreve a modelagem e o desenvolvimento de metodologia bem como a análise de resultados para consolidação da biblioteca Int-DWTs, contendo Métodos Intervalares para Transformadas Discretas Wavelet. Os métodos presentes em Int-DWTs incluem estensões intervalares e suas correspondentes otimizações para Transformadas Discretas Wavelet (TDWs), mais especificamente para as Transformadas Wavelet de Haar e Daubechies. As otimizações implementadas promoveram simplificações algébricas visando o incremento de desempenho e a garantia de exatidão das TDWs estudadas. São apresentadas e comparadas as análises de complexidade dos algoritmos desenvolvidos em relação aos presentes na literatura. A qualidade de resultados é verificada ao comparar três métricas: $EUC$ (\textit{Euclidean Distance}), $MSE$ (\textit{Mean Squared Error}) e $PSNR$ (\textit{Peak Signal-to-Noise Ratio}). O nível de erro produzido pelas formulações intervalares, como também os desempenhos obtidos através das simplificações propostas, são analisadas e comparadas com resultados baseados na literatura. Em contextos onde as escalas de representação são relevantes para o problema, os ganhos de exatidão obtidos com as otimizações propostas representam uma contribuição significativa para a área de Computação Científica.
...
In the last decade, many studies associating wavelet transforms with interval mathematics have arisen, promoting a new research direction and pointing out the relevance of  interval computations for a wide range of applications. As proposed by~\cite{shu-li2005}, the interval extension of the interpolating wavelet family for the construction of an adaptive algorithm for solving Partial Differential Equations (PDE).

Another collocation method for solving PDE is presented by~\cite{liu2013}, based on the interval extension for the Shannon-Garbor wavelet family. With respect to image processing applications, ~\cite{minamoto2010} proposes a blind digital image watermarking method using interval Daubechies wavelets.

The main motivation for interval techniques integrated with  DWTs is to provide trustworthy and validated results to technological and SC applications assuming such transformations. The techniques involving wavelets is ample, specially in signal and image processing~\cite{5,6}.

The  HWT, the  simplest Discrete Wavelet Transform~(DWT) due to the complexity of its filters, is still nowadays largely explored,  being its robustness with respect to many different mathematical structures and space formulations  a relevant aspect~\cite{novikov2012} addressed in many applications. Besides, as demonstrated in \cite{brito1998}, the imaging inaccuracy of the Haar wavelet basis is the smallest possible, motivating its usage in many 2D problems as well as the current formulation in the interval context. Preliminary results of the new interval extension of the HWT presented in~\cite{7} are revisited here

The optimal and interval extension of the DaWT is also addressed as one of the goals of the current work. In contrast to the HWT, the DaWT has more complex filters~\cite{DAUBTHESIS} requiring different algebraic manipulations in order to optimize its calculations. The developed optimizations ensures more exact results due to the reduced amount of irrational values to be computed during the transformation.

The current research addresses as well the {\it À-Trous} algorithm, an undecimated HWT formulation ~\cite{IEEE2007,kozakevicius2013}, proposing interval extensions for both  1D and 2D cases. Furthermore, a threshold procedure for treating signals, considered as a significant part of many compression and filtering algorithms, is analyzed and its interval extensions are presented, namely the Hard and Soft Decision procedures. 
...
Despite distinct approaches of arithmetic for  intervals, see e.g.~\cite{Walster97a,Hickey2001a} and~\cite{FS04}, this work considers the interval analysis proposed by Moore in the 1960's, providing methods to obtain accuracy in numerical calculations~\cite{Moore79,Moore59a,MoorePhD}.
%
...
Finally, it should be noticed that a computation of the MSE between two identical images, the value will be zero and hence the $PSNR$ will be undefined. Moreover, as the main limitation, both metrics relies strictly on numerical comparison, on which exactly focuses our study in this work.
...
The Haar basis was proposed in 1910, introduced by the Hungarian mathematician Alfred Haar \cite{haara} in a different approach than the one considered by \cite{daub1988}, when she presented her orthogonal basis with compact support for the space of square integrable functions\cite{daubechies1992}, called wavelets.
...
Both $ C^j_R $ and $ C^j_C $ store scalar coefficients, while $ D^j_R $ and $ D^j_C $ hold wavelet coefficients, and those four matrices represent one level full decomposed. The À-Trous Non Standard algorithm is shown in Figure~\ref{fig:AtrousNonStandardPseudo}.
...
Looking at the original decomposition procedure, shown in Figure~\ref{fig:DandDS}, the number of operations of the $DecompositionStep$ is $n + 1$, and for the main $ decomposition $ algorithm the number of operations is $n + \log(n) \cdot (n + 2) + 1$. By the proposed simplification they are reduced to $n + n\log(n)$ operations. By performing the non-normalized decomposition there is no need to execute the normalization step at line 1 of the original $decomposition$ algorithm, neither the divisions by $\sqrt{2}$ at its step procedure. Therefore, overall number of operations for non-normalized transformation of a $n$ sized vector is $n\log(n)$, representing the pseudo-code shown in Figure~\ref{fig:NewDandDS}.
...
The normalization procedure, presented in Figure~\ref{fig:1DNorm_and_Denorm}, performs the normalization step of all coefficients after their convolution using filters. Since it is executed once on every coefficient of a $n$ sized vector its number of operations is $n$.

By performing a non-normalized transform and the normalization procedure afterwards, the number of operations is $n\log(n) + n$, which is the same amount of operations from the normalized transformation.
...
The original algorithms for composition and its step procedure, Figure~\ref{fig:CandCS}, behave similarly to decomposition. The number of operations of the $Composition$ procedure is $n\log(n) + n$ if the normalized formulation is considered, otherwise it is $n\log(n)$. The denormalization step, also shown in Figure~\ref{fig:1DNorm_and_Denorm}, is almost the same $Normalization$ algorithm, whose complexity is $n$.

The difference is at line 6, where it is dividing each coefficient by the normalization factor, preparing the data to be transformed. Therefore the optimal composition has the same amount of operations from the original version, $n\log(n)$, but its results are more exact due to the normalization step, avoiding the calculation of $\sqrt{2}$ on every iteration.
...
By analyzing the original standard algorithm, Figure~\ref{fig:SDandND}, it depends on the original normalized decomposition procedure, whose number of operations is $n\log(n) + n$, executing it for every row and column of the input matrix.

Considering a $n$x$n$ matrix the number of operations of the original standard algorithm is $n\cdot(n\log(n) + n) + n\cdot(n\log(n) + n)$, therefore $2n^2\log(n) + 2n^2$. Performing the non-normalized decomposition algorithm, discussed in Section~\ref{subsec:1DCascadeOpt}, it reduces the calculations by avoiding intermediate normalization steps.
...
The non-normalized standard decomposition is performed with $2n^2\log(n)$ operations, and the last step is to normalize the results using the $StandardNormalization$ algorithm, presented in Figure~\ref{fig:StandardNormAlg}, and since it is operating one time on each coefficient of the input matrix its number of operations can be expressed by $n^2$.

The number of operations by performing the non-normalized standard decomposition and the standard normalization step can be expressed by $2n^2\log(n) + n^2$, which is faster than the original normalized procedure by avoiding a second iteration of $n^2$ operations on the input matrix.

The original normalized standard composition, presented in Figure~\ref{fig:InverseSCandNSC}, can be analyzed in the same way as its decomposition procedure since it performs the same number of operations but produces an approximation of the input matrix used for decomposition. The original version is executed in $2n^2\log(n) + 2n^2$ time, while its optimized version operates in $2n^2\log(n) + n^2$.
...
Considering a quadratic matrix of order $n$, the rest of the algorithm operates in a $while$ loop which is executed $\log_2(n)$ times. For each $while$ loop there are two $for$ loops also executing $\log_2(n)$ times, each one performing the $DecompositionStep$ algorithm ($n$ operations) on portions of the input matrix, depending on the level of transformation. The original non-standard decomposition is executed with $n^2 + \log(\log(n) + \log(n))$ operations.
...
The non-normalized non-standard decomposition is performed on $\log(\log(n) + \log(n))$ operations,  and its $Non-StandardNormalization$ algorithm, shown in Figure~\ref{fig:NonStandardNormAlg}, is performed on $n^2$ operations.

The number of operations of these procedures is $n^2 + \log(\log(n) + \log(n))$, which is the same from the original normalized non-standard algorithm. The gain of time presented in Figure~\ref{fig:test2S-NS} was obtained performing simple divisions by $2$ which are faster than using $\sqrt{2}$ as divisor when analysing processor microinstructions.

\newpage

\subsection{Undecimated Formulation}\label{sec:hwt-undec}

This Section presents the developed optimizations for both one and two dimensional approaches of the HWT À-Trous in Sections~\ref{sec:hwt-undec-1D} and \ref{sec:hwt-undec-2D}.

\subsubsection{One Dimensional}\label{sec:hwt-undec-1D}
...
The number of operations of both $ DecompositionStep $ and $ Decomposition $ procedures are  $2n$ and $Levels\cdot 2n$, respectively. Both are part of the original decomposition procedure, shown in Figure~\ref{fig:AtrousDecompMethods}. As in the previous cases, by performing a non-normalized decomposition there is no need to execute divisions by $\sqrt{2}$ in the step procedure, pseudo-code shown in Figure~\ref{fig:NewAtrousDS}, reducing the corresponding amount of operations is $2n$ and $2n\cdot Levels$, the same complexity as the original normalized transform.
...
The normalization procedure multiplies each scalar coefficient by its normalization factor, as shown in Eq.~(\ref{eq:NormFac}), for every resultant vector from the transform, configuring $ n\cdot Levels$ operations from line 1 to 5 of the pseudo-code. With the new set of scalar values all wavelet coefficients are then recalculated, configuring $ Levels\cdot(n + 3) + 2$ from line 6 to 16, which can be simplified to $ n\cdot Levels$. The overall amount of operations of the procedure is $2n\cdot Levels$.
...
By performing a non-normalized vector transform and the normalization procedure, the number of operations is $4n\cdot Levels$, which is more expensive than the original normalized formulation. The gain obtained with the new implementation is on the accuracy of results by avoiding high number of divisions using $\sqrt{2}$ from the original $DecompositionStep$ algorithm, as shown in Figure~\ref{fig:test1-3}.

The composition process, shown in Figure~\ref{fig:AtrousCompMethod}, starts with a copy of all scalar coefficients from the lowest level summing it with all wavelet vectors produced during decomposition. The number of operations of this procedure is $ n\cdot Levels) $ and can be used for both normalized and non-normalized transformations, there is no need for optimization.

\subsubsection{Two Dimension}\label{sec:hwt-undec-2D}
...
In Figure~\ref{fig:AtrousStandardPseudo} the original standard procedure is presented. It performs the decomposition of every line and level of a $n$x$m$ matrix, configuring an amount of $n\cdot 2m\cdot Levels$ operations.

The analog procedure for all columns of the same matrix has $m\cdot 2n\cdot Levels$. The entire procedure is expressed by $n\cdot 2m\cdot Levels + m\cdot 2n\cdot Levels$, which can be simplified to $4n\cdot m\cdot Levels$.
...
The next step in order to normalize the transformed values is to perform the $A\-TrousMatrixNormalization$ procedure, shown in Figure~\ref{fig:AtrousMatrixNormalization}, which is executed with $4n\cdot m\cdot Levels$ operations.

Therefore, the optimized decomposition procedure developed costs $8n\cdot m\cdot Levels$, twice the cost of the original normalized formulation, but tests indicate that our approach results in more exact values as shown in Figure~\ref{fig:test4S-NS}.

\renewcommand{\arraystretch}{1.2}

\begin{table}[h!]
\centering
\caption{Number of operations of the Decimated HWT.}
\label{tab:Complexity-HWT-Decimated}
\begin{tabular}{l|l|l|l}
             & 1D             & 2D Standard           & 2D Non-Standard       \\ \hline
Original     & $n\log(n) + n$ & $2n^2\log(n) + 2n^2$  & $n^2 + \log(\log(n) + \log(n))$\\
developed    & $n\log(n) + n$ & $2n^2\log(n) + n^2$   & $n^2 + \log(\log(n) + \log(n))$          
\end{tabular}
\end{table}

\begin{table}[h!]
\centering
\caption{Number of operations of the Undecimated HWT for decomposition.}
\label{tab:Complexity-HWT-Undecimated}
\begin{tabular}{l|l|l}
             & 1D                & 2D                    \\ \hline
Original     & $2 \cdot n \cdot Levels$ & $4 \cdot n \cdot m \cdot Levels $  \\
developed    & $4 \cdot n \cdot Levels$ & $8 \cdot n \cdot m \cdot Levels $          
\end{tabular}
\end{table}

The Tables~\ref{tab:Complexity-HWT-Decimated} and \ref{tab:Complexity-HWT-Undecimated} present a comparison of the studied algorithmic complexity for both approaches of the WHT. The expressions show that the developed 1D and 2D Non-Standard HWT Decimated are executed with the same amount of operations compared to their original formulation. The developed 2D Standard HWT is performed with less operations than the original algorithm from the literature. The developed HWT Undecimated needs more operations to be performed comparing to the one from the literature.
...
Looking at the original decomposition procedure, shown in Figure~\ref{fig:DaubDandDS}, the number of operations of the $DecompositionStep$ is $3 + (n/2)\cdot 4 + 3$, which can be simplified to $\BigO(n)$, and for the main $Decomposition $ algorithm the amount of operations is $1 + \log(n)\cdot (n + 1)$, that can be simplified to $\BigO(n\log(n))$.

The number of operations of the developed algorithms for decomposition, shown in Figure~\ref{fig:Daub_Opt_D_and_DStep}, are expressed by $5 + (n/2)\cdot 9 + 8$, or just $n$, for the developed $DecompositionStep$ procedure and $1 + \log(n)\cdot (n + 1)$, or just $n\log(n)$ for the developed $Decomposition$ routine.
...
Since it is executed once on every coefficient of a $n$ sized vector, its complexity is expressed by $n$. Performing a non-normalized transformation and the normalization procedure afterwards, the number of operations is $n\log(n) + n$, making the developed optimization slower than the original formulation.
...
In the 1D DaWT composition procedure and its step procedure, as shown in Figure~\ref{fig:DaubCandCS}, behave similarly to decomposition where its number of operations is expressed by $n\log(n)$.

The developed optimal $Composition$ procedure, shown in Figure~\ref{fig:Daub_Opt_C}, shares the same number of operations of $n\log(n)$, but its results are more exact due to the simplified filters, avoiding the unnecessary calculation of $\sqrt{2}$ and $\sqrt{3}$ on every iteration.
...
The analysis of the original Standard algorithm inherited from the HWT depends on the original normalized decomposition procedure, as shown in Figure~\ref{fig:SDandND}. By executing such algorithm for every row and column of the input matrix, one can observe the corresponding number of operations analysis as $n\log(n)$. Considering a matrix $C_{n \times n}$, the number of operations of the original standard algorithm is $n(n\log(n)) + n(n\log(n))$, or just $n^2\log(n)$.
...
\begin{table}[h!]
\centering
\caption{Number of operations of the DaWT for decomposition.}
\label{tab:Complexity-DaWT}
\begin{tabular}{l|l|l|l}
             & 1D             & 2D Standard        & 2D Non-Standard       \\ \hline
Original     & $n\log(n)$     & $n^2\log(n)$       & $n^2 + \log(\log(n) + \log(n))$\\
developed    & $n\log(n) + n$ & $n^2\log(n) + n^2$ & $n^2 + \log(\log(n) + \log(n))$          
\end{tabular}
\end{table}

\newpage

The Table~\ref{tab:Complexity-DaWT} shows that the developed DaWT is perform using more operations compared to its original formulation based on the literature.
...
The Hard Decision is made by verifying if the interval data is entirely less than $ \tau $, comparing the infimum of the information and the supremum of the interval parameter. This procedure grants that every possible punctual coefficient is less than all possible punctual values belonging to $ \tau $.

The Soft Decision is made by verifying if most of the interval data is less than the midpoint of $ \tau $, comparing the infimum of the information and the center of the threshold. This procedure grants that most possible punctual coefficients are between the punctual values of $ \overline{\tau} $ and $ \underline{\tau} $.
...
\begin{figure}[h!]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test1.pdf}
	\caption{Decimated 1D HWT}
    \label{fig:Dec1DHWT}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test3.pdf}
...
\begin{figure}[h!]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test2S.pdf}
	\caption{Decimated 2D Standard HWT}
    \label{fig:Dec2DSHWT}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test2NS.pdf}
...
\begin{figure}[h!]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test4S.pdf}
	\caption{Undecimated 2D Standard HWT}
    \label{fig:Und2DSHWT}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test4NS.pdf}
...
\begin{figure}[h!]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test5-1048576.pdf}
	\caption{Using 1048576 values}
    \label{fig:1DDaWT-1}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test5-4194304.pdf}
	\caption{Using 4194304 values}
	\label{fig:1DDaWT-2}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test5-16777216.pdf}
...
\begin{figure}[h!]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test6S-1024.pdf}
	\caption{Using 1024x1024 matrix}
    \label{fig:2DSDaWT-1}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test6S-2048.pdf}
	\caption{Using 2048x2048 matrix}
	\label{fig:2DSDaWT-2}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test6S-4096.pdf}
...
\begin{figure}[h!]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test6NS-1024.pdf}
	\caption{Using 1024x1024 matrix}
    \label{fig:2DNSDaWT-1}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test6NS-2048.pdf}
	\caption{Using 2048x2048 matrix}
	\label{fig:2DNSDaWT-2}
  \end{subfigure}
  \begin{subfigure}[b]{0.48\textwidth}
	\includegraphics[width=\textwidth]{Imagens/test6NS-4096.pdf}
	\caption{Using 4096x4096 matrix}
